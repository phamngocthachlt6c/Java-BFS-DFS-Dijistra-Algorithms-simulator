...
import graphics.SoDo;
import status_scenes.BfsStatusScene;
import status_scenes.StatusScene;
...
public class panelSoDoBFS extends JPanel implements ActionListener {
	...
	public void makeKichBanBFS(int xuatphat, int n) {
		...
		kBan.insertSceneStatus("buff", 0, 0, queue, nodeDangXet, 0, nEdge,0,0,"",soDinhDaDuyet);
		int f = 0, r = 0;
		kBan.insertSceneStatus("begin", f, r, queue, nodeDangXet,0, nEdge,0,0,"",soDinhDaDuyet);
		kBan.insertSceneStatus("Khoi tao", f, r, queue, nodeDangXet,0, nEdge,0,0,"", soDinhDaDuyet);
		queue[r++] = xuatphat;
		tham[xuatphat] = 1;

		kBan.insertSceneStatus("B1", f, r, queue, nodeDangXet, 0, nEdge,0,0,"", soDinhDaDuyet);
		kBan.insertSceneStatus("fKhacrFirst", f, r, queue, nodeDangXet, 0,nEdge,0,0,"", soDinhDaDuyet);
		...
		while (f != r) {
			...
			System.out.print(" " + i);
			kBan.insertSceneStatus("B2", f, r, queue, nodeDangXet, 0, nEdge,i,1,kqDuyet,soDinhDaDuyet);
			System.out.println("Nut dang xet: " + nodeDangXet);
			kBan.insertSceneStatus("jNhoHonNFirst", f, r, queue, nodeDangXet, 0,nEdge,i,1,kqDuyet,soDinhDaDuyet);
			for (int j = 1; j <= n; j++) {
				
				kBan.insertSceneStatus("daTham", f, r, queue, nodeDangXet, j,nEdge,i,j,kqDuyet, soDinhDaDuyet);
				...
				if (tham[j] == 0 && a[i][j] != 0 && a[i][j] != 1111) {
					...
					edgeL.InsertEdge(0, mf.wp.nodeLinkData.get_Node(nodeDangXet-1), mf.wp.nodeLinkData.get_Node(j-1));
					nEdge++;
					//******************** =))**********************
					kBan.insertSceneStatus("B3", f, r, queue, nodeDangXet, j, nEdge,i,j,kqDuyet, soDinhDaDuyet);
					kBan.insertSceneStatus("plusJFirst", f, r, queue, nodeDangXet, 0, nEdge,i,j+1,kqDuyet, soDinhDaDuyet);
				} else
					kBan.insertSceneStatus("plusJSecond", f, r, queue, nodeDangXet, 0, nEdge,i,j+1,kqDuyet, soDinhDaDuyet);
				kBan.insertSceneStatus("jNhoHonNSecond", f, r, queue, nodeDangXet, 0, nEdge,i,j+1,kqDuyet, soDinhDaDuyet);
			}
			kBan.insertSceneStatus("fKhacrSecond", f, r, queue, 0, 0, nEdge,i,0,kqDuyet, soDinhDaDuyet);
		}
		kBan.insertSceneStatus("end", f, r, queue, 0, 0, nEdge,0,0,kqDuyet, soDinhDaDuyet);
		kBan.insertSceneStatus("buff2", f, r, queue, nodeDangXet, 0, 0,0,0,kqDuyet,soDinhDaDuyet);
		...
		*/
			...
			if(dbImage!=null){
				...
				if(g2!=null){
					...
					mf.wp.gp.drawHideGraph(g2, mf.wp.nodeLinkData);
					//Node******
					
					mf.wp.gp.drawEdgeListSapDuyet(g2, kBan.edgeL,buoc.nEdge,mf.wp.nodeLinkData);
					
					
					
					for(int i=0;i<buoc.r;i++){
						mf.wp.drawN.nodeSapDuyet(g2, mf.wp.nodeLinkData.get_Node(queue[i]-1).x,
								mf.wp.nodeLinkData.get_Node(queue[i]-1).y, queue[i]-1);
					}
					mf.wp.gp.drawEdgeList(g2, kBan.edgeL, buoc.soDinhDaDuyet-1, mf.wp.nodeLinkData);
					
					mf.result.setResult(buoc.kqDuyet);
					
					if (mf.wp.nodeLinkData.get_xy(buoc.nodeDangXet - 1) != null
							&& buoc.nodeDangXet != 0&&g2!=null) {
						
						
						
						mf.wp.drawN.nodeStart(g2, mf.wp.nodeLinkData
						...
						if(buoc.nodeKeDangXet!=0){
							...
							mf.wp.drawN.nodeKeDangXet(g2, mf.wp.nodeLinkData.get_Node(buoc.nodeKeDangXet-1), false,true);
							else 
								mf.wp.drawN.nodeKeDangXet(g2, mf.wp.nodeLinkData.get_Node(buoc.nodeKeDangXet-1), false,false);
						}
					...
					if(!buoc.kqDuyet.equals("")&&buoc.nodeDangXet-1!=mf.wp.nodeLinkData.FirstNode.cost
							&&buoc.nodeKeDangXet-1!=mf.wp.nodeLinkData.FirstNode.cost)
						mf.wp.drawN.nodeRoot(g2, mf.wp.nodeLinkData.FirstNode, false);
					//************************
					
					
					if(run==false&&buoc==null) mf.wp.gp.drawResultBFS(g2, mf.wp.nodeLinkData);
