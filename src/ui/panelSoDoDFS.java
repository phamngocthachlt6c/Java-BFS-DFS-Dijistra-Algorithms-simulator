...
import graphics.SoDoDFS;
import status_scenes.DfsStatusScene;
import status_scenes.StatusScene2;
...
public class panelSoDoDFS extends JPanel implements ActionListener{
	...
	public void makeKichBanDFS(int xuatphat, int n) {
		...
		while(topS>0){
			...
			if(back[v]!=0) listEdge.InsertEdge(0, mf.wp.nodeLinkData.get_Node(v-1), mf.wp.nodeLinkData.get_Node(back[v]-1));
		...
		Graphs d = new Graphs();
		Edge_List list = d.getEdgeDFS(mf.wp.nodeLinkData);
	...
	public void setState() {
		...
		if(dbImage!=null){
			...
			if(g2!=null){
				...
				if(buoc.nEdge>0) {
					mf.wp.gp.drawEdgeList(g2, kBan.edgeL, buoc.nEdge, mf.wp.nodeLinkData);
				...
				if(buoc==null&&buoc.buoc=="buff2") mf.wp.gp.drawResultDFS(g2, mf.wp.nodeLinkData);
