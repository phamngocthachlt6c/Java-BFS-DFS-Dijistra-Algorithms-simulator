...
import logic.CuScrollBarX;
import logic.CuScrollBarY;
import logic.TapDinh;
import logic.resultDijkstra;
import logic.tableDJ;
import logic.TapDinh.Dinh;
import status_scenes.StatusScene3;
import status_scenes.kichBanDJ;
import status_scenes.kqDuyet;
public class panelDijstra extends JPanel implements ActionListener,
	...
	public Graphics2D dbg2;
	public Image dbImage;

	public kichBanDJ kBan;
	public StatusScene3 buoc;
	public TapDinh tapDinh;
	...
	public void makeKichBan(int n) {
		...
		while(r<=n){
			...
			if(tapDinh.getVisDMin()!=null) {
				edgeL.InsertEdge(0, mf.wp.nodeLinkData.get_Node(tapDinh.getVisDMin().path-1),
						mf.wp.nodeLinkData.get_Node(tapDinh.getVisDMin().dinh-1));
				...
				if(buoc!=null){
					if(buoc.text!="Init"&&buoc.text!="(∞,-)"&&buoc.text!="(0,-)"&&buoc.text!="buff")
						mf.wp.drawN.nodeRoot(g2, 
							mf.wp.nodeLinkData.get_Node(0).x,mf.wp.nodeLinkData.get_Node(0).y, 0);
					mf.wp.gp.drawEdgeList(g2, kBan.edgeL,buoc.edgeN,mf.wp.nodeLinkData);
