...
import graphics.resultSearch;
import logic.Node_Link;
import logic.TableValues;
import status_scenes.kichBanDJ;
...
public class MainFrame extends JFrame implements ActionListener {
	...
	JButton edit = new JButton("Edit graph");
	...
	private JPanel panelSuadoThi = new EditGraphRightPanel();
	...
	// ///////////////////////////
	// MENU
	JButton New, Edit, BFS, DFS, DIJStra, saveGraph;
	// statute
	...
	public MainFrame() {
		...
		saveGraph = new JButton("Save The Graph");
	...
	private void makeDescr() {
		...
		batdau.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!panelSoDoBFS.run){
					...
					if(wp.nodeLinkData.FirstNode!=null){
		...
		tiepTheo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if (panelSoDoBFS.buoc == null && wp.nodeLinkData.FirstNode != null) {
					panelSoDoBFS.makeKichBanBFS(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
					panelSoDoBFS.buoc = panelSoDoBFS.kBan.start;
				}
		...
		tiepTheo2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if (panelSoDoDFS.buoc == null && wp.nodeLinkData.FirstNode != null) {
					panelSoDoDFS.makeKichBanDFS(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
					panelSoDoDFS.buoc = panelSoDoDFS.kBan.start;
				}
		...
		tiepTheo3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if (panelDijstra.buoc == null && wp.nodeLinkData.FirstNode != null) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == New) {
			...
			wp.nodeLinkData = new Node_Link();
			wp.repaint();
			wp.data = 0;
			wp.matNum.updateSize(wp.nodeLinkData);
			wp.updateTable();
		...
		if (e.getSource() == this.BFS) {
			...
			result.setVisible(true);
				...
				if (wp.nodeLinkData.FirstNode != null){
					panelSoDoBFS.setKhoiTao(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
					panelSoDoBFS.makeKichBanBFS(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
		...
		if (e.getSource() == this.DFS) {
			...
			result.setVisible(true);
				...
				if (wp.nodeLinkData.FirstNode != null){
					...
					panelSoDoDFS.setKhoiTao(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
					panelSoDoDFS.makeKichBanDFS(wp.nodeLinkData.FirstNode.cost + 1,
							wp.data);
